...
final class AnterosInMemoryObjectQueue<T> extends AnterosObjectQueue<T> {
  private final Deque<T> entries;
  /**
   * The number of times this file has been structurally modified — it is incremented during {@link
   * #remove(int)} and {@link #add(Object)}. Used by {@link AnterosInMemoryObjectQueue.EntryIterator} to
   * guard against concurrent modification.
   */
  @Private int modCount = 0;
  @Private boolean closed;

  AnterosInMemoryObjectQueue() {
  ...
  private final class EntryIterator implements Iterator<T> {
    ...
    @Override public void remove() {
      ...
      AnterosInMemoryObjectQueue.this.remove();
