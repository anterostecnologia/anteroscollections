package br.com.anteros.collections.queue.object;
...
/** A queue of objects. */
public abstract class AnterosObjectQueue<T> implements Iterable<T>, Closeable {
  /** A queue for objects that are atomically and durably serialized to {@code file}. */
  public static <T> AnterosObjectQueue<T> create(QueueFile qf, Converter<T> converter) {
    return new AnterosFileObjectQueue<>(qf, converter);
  }
   ...
   */
  public static <T> AnterosObjectQueue<T> createInMemory() {
    return new AnterosInMemoryObjectQueue<>();
  }
